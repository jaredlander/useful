...
test_that('All functions return the correct type', {
  ...
  expect_is(FitKMeans(iris[, -ncol(iris)], algorithm='Forgy'), 'data.frame')
  #expect_is(FitKMeans(iris[, -ncol(iris)], spectral=TRUE, algorithm='Lloyd'), 'data.frame')
  expect_error(FitKMeans(iris[, -ncol(iris)], algorithm='lol'), cat("'arg' should be one of \"Hartigan-Wong\", \"Lloyd\", \"Forgy\", \"MacQueen\"", '\n'))
